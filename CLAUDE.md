...
the MCP-call trace) done; T4.4 (web screens) + T4.5 (latency) next.
...
| `apps/api`                     | Fastify + tRPC. Wires the packages; `/health`, `/auth/silpo/callback` (OAuth redirect → session), tRPC routers `auth` / `household` / `plan` / `recipes` (+ `hello`). `context.ts` resolves the session → `ctx.householdId` / per-household `ctx.retail`; `protectedProcedure` gates everything account-scoped. `src/{auth-flow,session,retail,mapper,plan,cart,mcp-trace,llm}.ts` + `household/` (bootstrap) + `queue/` (BullMQ). tRPC routers also include `cart` / `ops` (`ops.trace` = the T4.3 MCP-call trace). `src/scripts/{mcp-auth,mcp-register,mcp-tools-snapshot,mcp-audit,mcp-audit-cart,plan-probe,mapper-probe,cart-probe,plan-edit-probe,ops-trace}.ts` (`audit-util.ts` = tested pure helpers). |
