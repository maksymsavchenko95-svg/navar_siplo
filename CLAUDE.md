...
| `packages/retail`              | `@navar/retail` — the `RetailProvider` seam + Silpo MCP adapter + OAuth + `mcp:auth` flow. **The only package that imports `@modelcontextprotocol/sdk`** (ADR-04); a grep enforces it. |
...
| `packages/mapper`              | `@navar/mapper` — ingredient ↔ SKU matching. Stub (TDD §5).                                                                                                                            |
...
| `apps/api`                     | Fastify + tRPC. Thin: wires the packages, `/health`, tRPC router (`hello`, `recipes.list`, `mcp.listTools`), `src/scripts/{mcp-auth,mcp-tools-snapshot}.ts`.                           |
| `apps/web`                     | Next.js (App Router). One page calling the three tRPC procedures. Runs on the host.                                                                                                    |
